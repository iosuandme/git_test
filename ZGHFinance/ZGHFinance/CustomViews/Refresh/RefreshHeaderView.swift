...
class RefreshHeaderView: RefreshBaseView {
    class func footer(needTop : Bool)->RefreshHeaderView{
        let footer:RefreshHeaderView  = RefreshHeaderView(frame: CGRect(x :0, y :0, width : UIScreen.mainScreen().bounds.width, height :CGFloat(RefreshViewHeight)), needTop : needTop)
        return footer
    }
    ...
    override init(frame: CGRect , needTop : Bool = false) {
        super.init(frame: frame , needTop : needTop)
        lastUpdateTimeLabel = UILabel()
        lastUpdateTimeLabel.autoresizingMask = UIViewAutoresizing.FlexibleWidth
        lastUpdateTimeLabel.font = UIFont(name: "AvenirNext-Bold", size: 12)
